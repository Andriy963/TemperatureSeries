...
public class TemperatureSeriesAnalysis {
	...
	public int quantityOfActualTemperatures;
    ...
    public double average(){
		...
		double averageTemperature = sumOfTemperatures/quantityOfActualTemperatures;
    ...
    public double deviation(){
		...
		for(int i=0;i<quantityOfActualTemperatures;i++){
			double val = temperatures[i];
			sumOfSquares = sumOfSquares + (val - averageOfTemperatures)*(val - averageOfTemperatures);
		}
		double temperaturesDeviation = Math.sqrt(sumOfSquares/quantityOfActualTemperatures);
    ...
    public double findTempClosestToValue(double tempValue){
		...
		if(closestToValueElement >= tempValue){
			absSubOfValueAndClosestElement = closestToValueElement - tempValue;
		}
		else{
			absSubOfValueAndClosestElement = tempValue - closestToValueElement;
		}
		
		for(int i=0;i<quantityOfActualTemperatures;i++){
			double val = temperatures[i];
			if(val >= tempValue && (val - tempValue) < (absSubOfValueAndClosestElement - epsilon)){
				closestToValueElement = val;
				absSubOfValueAndClosestElement = val - tempValue;
			}
			else if(val < tempValue && (tempValue - val) < (absSubOfValueAndClosestElement - epsilon)){
				closestToValueElement = val;
				absSubOfValueAndClosestElement = tempValue - val;
			}
			else if(val >= tempValue && ((val - tempValue) - absSubOfValueAndClosestElement) < epsilon){
				closestToValueElement = val;
			}
		}
    ...
    public double[] findTempsLessThen(double tempValue){
		...
		int[] pointerOnGoodValues = new int[quantityOfActualTemperatures];
		...
		double[] valuesLessThenTempValue = new double[quantityOfGoodValues];
		int count = 0;
		for(int i=0;i<quantityOfActualTemperatures;i++){
    ...
    public double[] findTempsGreaterThen(double tempValue){
		...
		double[] valuesGreaterThenTempValue = new double[quantityOfGoodValues];
		int count = 0;
		for(int i=0;i<quantityOfActualTemperatures;i++){
			if(pointerOnGoodValues[i] == 1){
				valuesGreaterThenTempValue[count] = temperatures[i];
				count++;
			}
		}
    ...
    public TempSummaryStatistics summaryStatistics(){
		...
		TempSummaryStatistics summaryStatistic = new TempSummaryStatistics(avgTemp, devTemp, minTemp, maxTemp);
    ...
    public int addTemps(double ... temps){
		...
		if((temperatures.length - quantityOfActualTemperatures) >= temps.length){
			...
			while(newLengthOfArray < quantityOfActualTemperatures + temps.length){
				newLengthOfArray = newLengthOfArray*2;
			}
			double[] newTemperatureArray = new double[newLengthOfArray];
			int iter = 0;
